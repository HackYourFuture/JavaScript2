...

{
  ...
  function getBookInfo() {
    for (let key in myBooks) {
      ...
      let authorName = myBooks[key]['author']['name'];
      let authorAge = myBooks[key]['author']['age'];
      let authorCountry = myBooks[key]['author']['country'];
      let gender = myBooks[key]['author']['gender'];

      //if gender is female then return she
      const genderFinder = (gender) => {
        if (gender === 'female') {
          return 'she'
        } else {
          return 'he'
      ...
      authorInfo.innerHTML = `${authorName} is originally from ${authorCountry}, ${genderFinder(gender)} has died at the age of ${authorAge}.`;
      li.appendChild(authorInfo);

      // Add Book info
      let bookLanguage = myBooks[key]['book_info']['language']['mainLanguage'];
      let publishYear = myBooks[key]['book_info']['publication'];
      let translateNum = myBooks[key]['book_info']['language']['translations'];
