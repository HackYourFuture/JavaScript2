...

{
  ...
  function booksGenerator2() {
    const bookcolection = document.getElementById('books_collections');
    for (let i = 0; i < books.length; i++) {
      const bookInfo = booksInformation[books[i]];
      const booksDiv = document.createElement('div');
      booksDiv.setAttribute('id', books[i]);
      ...
      bookAuthor.innerText = 'written by: ' + bookInfo.author;
      booksDiv.appendChild(bookAuthor);
      const bookLanguage = document.createElement('h3');
      bookLanguage.innerText = 'language of book: ' + bookInfo.language;
